...
const StepTHree = () => {
    ...
    const [value, setValue] = useState(userData.loan_amount || ''); // Initial value of the slider
      ...
      function getCookie(name) {
         ...
         if (parts.length === 2) return parts.pop().split(";").shift();
              ...
              const handleChange = (event) => {
                ...
                SetUserData((prevUserData) => ({ ...prevUserData, loan_amount: event.target.value }));
              };
              ...
              const handleNext = async () => {
                if (value) {
                    ...
                    // Updating local context state
                    SetUserData((prevUserData) => ({ ...prevUserData, loan_amount: value }));
        
                    // Define API URL
                    const apiUrl = `proxy?url=http://127.0.0.1:8080/loan/update/${userId}`;
        
                    try {
                        // Make an API call to save the loan amount
